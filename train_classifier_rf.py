...
for trees in n_estimators_list:
    for depth in max_depth_list:
        ...
        results_list.append({'n_estimators (樹數)': trees, 'max_depth (深度)': depth, 'Accuracy (%)': round(acc*100, 2), 'Macro Prec (%)': round(mac_prec*100, 2), 'Macro Rec (%)': round(mac_rec*100, 2), 'Macro F1 (%)': round(mac_f1*100, 2), 'Time (s)': round(train_time, 2)})
        current_run += 1

df_results = pd.DataFrame(results_list).sort_values(by='Macro F1 (%)', ascending=False)
df_results.to_excel(os.path.join(current_dir, "rf_grid_results.xlsx"), index=False)
print("\n✅ RF 測試完畢！報表已儲存至 rf_grid_results.xlsx")